...
void createThreads_bad(void) {
    ...
    for (size_t i = 0; i < NUM_THREADS; i++) {
        indexes[i] = i;     // universe.h 中声明的数组，表示哲学家和餐叉的序号
...
void createThread_correct1(void) {
    ...
    for (size_t i = 0; i < NUM_THREADS; i++) {
        indexes[i] = i;
        ret = pthread_create(&threadid[i], NULL, philosopher1
                          , (void*) &(indexes[i]));
        if (ret) {
            printf("Error: 创建线程出错\n");
            exit(1);
...
void createThread_correct2(void) {
    ...
    for (size_t i = 0; i < NUM_THREADS; i++) {
        indexes[i] = i;
        ret = pthread_create(&threadid[i], NULL, philosopher2
                          , (void*) &(indexes[i]));
        if (ret) {
            printf("Error: 创建线程出错\n");
            exit(1);
